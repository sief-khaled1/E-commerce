...
export default async function SpecificCategory({ params }: { params: Params }) {
    ...
    const response = await fetch('https://ecommerce.routemisr.com/api/v1/products?category[in]=' + SpecificCategory)
    const { data }: specCategory = await response.json();
    console.log(data)
    return <>
        {data[1] === undefined ? <div className='h-screen flex justify-center items-center'><p className='text-black/50 text-center text-4xl'>No products found in this category.</p></div> : <div className='md:p-30 p-10 '>
